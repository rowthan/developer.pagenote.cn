      ...
      <tbody>
        {commands?.map((command) => (
          <tr key={command.name}>
            ...
            <td>
              {command.shortcut ? (
                <kbd className="kbd kbd-sm text-neutral">
                  {command.shortcut}
                </kbd>
              ) : (
                <span>-</span>
        ...
        <tr>
          <td className={'text-sm'}>
            ...
            {keyupTimeout > 0 && (
              <div className={'text-xs text-gray-400'}>
                长按{' '}
                <a className={'link'} href="/ext/popup.html#/setting/light">
                  {(keyupTimeout / 1000).toFixed(1)}s
                </a>
              </div>
            )}
          </td>
          <td className={'whitespace-normal w-40'}>
            {shortcuts.map((item, index) => (
              <kbd
                ...
                className="kbd kbd-sm text-neutral"
              >
                {item.shortcut}
              </kbd>
            ))}
          ...
          <td>
            <a
              className={'text-xs link link-primary'}
              href={`${basePath}/pagenote.html#setting`}
              target={'_blank'}
            >
              修改
            </a>
