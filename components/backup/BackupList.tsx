    ...
    function loadList() {
        listBackupList().then(function (res) {
            const list = res.map(function (item) {
                return pick(item,['backupId','backup_at','remark','dataType','version','extension_version'])
            })
            setBackupList(list)
        })
    }
            ...
            <div className="overflow-x-auto">
                <table className="table table-compact w-full min-h-100">
                                ...
                                <tr key={index}>
                                    ...
                                    <td>
                                        <div className="dropdown dropdown-left">
                                            <label tabIndex={0} className="btn m-1 btn-sm">使用此备份
                                            </label>
                                            <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                                                <li>
                                                    <button onClick={()=>downloadBackupAsFile(item.backupId as string)}
                                                            disabled={loading}
                                                ...
                                                <li>
                                                    <button disabled={loading} onClick={()=>removeBackup(item.backupId as string).then(()=>{loadList()})} className="m-2 btn btn-sm btn-warning">删除备份</button>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
                    ...
                    <tr>
                        ...
                        <th colSpan={2}>
                            <button disabled={loading} onClick={createNewBackup} className="btn btn-outline btn-success">创建新的 备份时间节点</button>
