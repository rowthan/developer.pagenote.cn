    ...
    function onCropFinished() {
        ...
        UploadImage().then(function ({client,cloud_space}) {
            ...
            canvas.current?.toBlob(function (blob) {
                ...
                client.put(`${cloud_space}/${fileName}`, new Blob([blob], { type: ContentType.jpeg })).then(res =>{
            ...
            <Modal keepNode={false} open={showModal} toggleOpen={setModal}>
                <div>
                    ...
                    </label>
                    {
                        ...
                        <ImageShape
                            key={uploadFileImg}
                            cropCallback={(element)=>{canvas.current = element}}
                            originImgSrc={uploadFileImg||""}>
                            <div className={'text-right my-2'}>
                                <button disabled={uploading} className={`btn btn-sm ${uploading?'loading':""}`} onClick={onCropFinished}>确定</button>
                            </div>
                        </ImageShape>
                    }
                </div>
