...
export default function SignUpForm() {
    ...
    if(signed){
        return (
            <div>
                你已经成功登录，请 <a className={'link'} onClick={signout}>退出后</a> 注册
            </div>
        )
    }
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div className="text-xs">
                            登录密码已发送至你邮箱，请查收后 <a className={'link'} href={`${basePath}/signin.html`}>登录</a>
                        </div>
                    </div>
                </div>
                <div className="flex-none">
                    {registerResult?.emailServer && <a href={registerResult.emailServer} className="btn btn-sm">查收邮件</a>}
                    <a className="btn btn-sm btn-primary" href={`${basePath}/signin.html`}>去登录</a>
                </div>
            </div>
        )
    }
    ...
    return (
        <form className='max-w-lg m-auto' onSubmit={handleSubmit((data) => onSubmit(data))}>
            ...
            <div className='flex justify-end mt-4 mb-10'>
                <button disabled={loading} className={`btn btn-sm btn-outline btn-info ${loading ? 'loading' : ''}`}
                        type="submit">
                    注册
                </button>
            </div>
